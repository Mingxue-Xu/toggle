#!/usr/bin/env python3
"""
Unified example: Activation-driven SVD on Gemma-3-27B-IT with optional CoLA backend.

Pipeline:
- Load model/tokenizer
- Profile baseline inference memory (random ids)
- ModelConsolidator SVD compression (CoLA or torch backend)
- Profile compressed inference memory (random ids)
- Report parameter counts and compression ratio (compressed/original)
- Save compressed model and manifest

Usage:
  HF_TOKEN=... python scripts/examples/svd_gemma3.py [--cola yes|no] [--config path] [--workspace path]
"""
...
def params_count(model) -> int:
    """Total parameter count (all parameters)."""
...
def _configure_linalg_backend():
    """Optionally override CUDA linalg backend via env for easier debugging."""
...
def _run_with_linalg_retry(fn, *, fallback_env="TORCH_LINALG_FALLBACK"):
    """
    Detect cuSolver-specific errors and retry once with an alternate backend.
    Helps distinguish backend issues from other failures without hiding errors.
    """
    try:
        return fn()
...
def _select_save_dir(cfg: dict, workspace: Path, compressed_subdir: str) -> Path:
    ...
    if compressed_dir:
        save_dir = _resolve_path(compressed_dir)
    else:
        save_dir = workspace / compressed_subdir
    return save_dir
