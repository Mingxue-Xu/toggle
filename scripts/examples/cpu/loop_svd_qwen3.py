#!/usr/bin/env python3
"""
Loop Qwen3 SVD compression over a list of ranks from CSV.

Reads rank specs from a CSV file and runs the Qwen3 SVD pipeline for each row,
mirroring the structure used by scripts/examples/svd_qwen3.py.
...
Usage:
  python scripts/examples/loop_svd_qwen3.py --input-csv path/to/ranks.csv
"""
...
def _objects_to_str(objects: list) -> str:
    if not objects:
        return ""
...
def run_single(cfg: dict, *, use_cola: bool, workspace: Path) -> dict:
    ...
    result = {
        ...
        "factorization_objects": _objects_to_str(objects),
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Loop Qwen3 SVD compression over ranks from CSV.")
    ...
    parser.add_argument(
        "--log-file",
        default=None,
        help="Optional text log file path (key\\tvalue lines per run). Defaults to ../logs/act-remote-eval/qwen3_svd_eval_rank{}.txt",
    )
    parser.add_argument(
        "--tasks",
        default=None,
...
def main() -> None:
    ...
    default_log = _resolve_path(args.log_file) if args.log_file else (ROOT.parent / "logs" / "act-remote-eval" / "qwen3_svd_eval_rank{}.txt")
    ...
    for idx, spec in enumerate(specs, start=1):
        ...
        if log_path:
            ...
            if "{}" in log_path_str:
                log_path = Path(log_path_str.format(spec.attention_rank))
        try:
            ...
            err_row = {
                ...
                "save_dir": "",
                "backend": "",
                "factorization_objects": _objects_to_str((cfg.get("factorization") or {}).get("objects") or []),
                "orig_model_params": "",
                "comp_model_params": "",
                "compression_ratio": "",
                "compression_sec": "",
            ...
            if log_path:
                _append_text_log(log_path, err_row)
            continue
