...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "Warning: HF_TOKEN is not set; model download may fail." >&2
fi
...
NAMES=(
  "svd_qwen3_no_cola"
  "svd_qwen3_cola"
  "svd_qwen3_compressed_no_cola"
  "svd_qwen3_compressed_cola"
)
...
for i in "${!COMMANDS[@]}"; do
  ...
  echo "  input_batch_size: ${input_bs:-n/a}"
  echo "  input_seq_len: ${input_seq:-n/a}"
