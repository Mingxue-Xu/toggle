...
The constructor creates `experiments.csv`, `models.csv`, `evaluations.csv`, and `compressions.csv` with headers on first use.
...
logger.end_experiment(status="completed")           # or "failed", "cancelled"
```
...
def _experiment_session(csv_logger, request):
    csv_logger.start_experiment(request.node.name, config_file="e2e_test")
    yield
...
`model_info` must contain `"num_parameters"` and `"size_mb"` -- these are extracted into dedicated CSV columns; the full dict is stored as JSON in `architecture_info`.
...
`compression_results` keys: `original_size_mb`, `compressed_size_mb`, `compression_ratio`, `compression_time`, `memory_usage_mb`. The `parameters` dict is stored as JSON.
...
`task_results` is flattened: `{task_name: {metric_name: value}}` produces one CSV row per metric. A non-dict value is stored with `metric_name="result"`.
...
Reads the CSV files back for cross-experiment analysis.
